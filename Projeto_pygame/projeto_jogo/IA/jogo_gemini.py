...
class Tanque:
    def __init__(self, x, y, cor, controles):
        ...
        # Calcula o ponto de origem do tiro (frente do tanque)
        self.cano_offset = self.imagem.get_width() // 2
    ...
    def atirar(self):
        ...
        if agora - self.ultimo_tiro > self.cooldown_tiro:
            # Calcula o ponto da ponta do canhão baseado no centro e ângulo
            distancia_cano = self.imagem.get_width() // 2
            ponta_x = self.x + math.cos(self.angulo) * distancia_cano
            ponta_y = self.y + math.sin(self.angulo) * distancia_cano
    ...
    def desenhar(self, tela):
        ...
        ret = imagem_rotacionada.get_rect(center=(self.x, self.y))
...
def main():
    ...
    while running:
        ...
        if jogo_ativo:
            ...
            texto1 = fonte.render("Jogador 1 (WASD + Espaço)", False, AZUL)
            texto2 = fonte.render("Jogador 2 (Setas + Enter)", False, VERDE_ESCURO)
